...
| Study page | `site/research/diffci-open-evidence-2026.html` | Same stylesheet and chrome as the three case studies; five inline SVG charts (three-way classification, conditional-vs-corpus, per-repository diverging bars, historical recall, incremental economics) using the site's data-viz slots plus one neutral slot. Every chart value names the figure id it must equal |
...
| Guards | [`../../tests/open-study/open-study.test.ts`](../../tests/open-study/open-study.test.ts) | Every internal source path exists; every cited report has a label; rendered files on disk are byte-identical to a fresh render; every chart value equals its figure or table cell; disclaimer, licence and the unflattering numbers are present; no internal path or `.md` reference leaks into any published artefact |
...
| Editorial article, published PDF | (none) | Removed from the site 2026-09-03 (founder decision: cite the study page, not a hosted article). The corrected typeset PDF exists offline; nothing on the site links or hosts it |
...
| Homepage | `site/index.html` | A fourth card in the evidence section and a footer link, both to the study only. Founder decision 2026-09-03: the article PDF is not linked from nav, cards or the study page |
